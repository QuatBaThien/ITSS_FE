...
export default {
  ...
  methods: {
    onChange(e) {
      ...
      this.coffeShop.image.push({ id: id++, image:file, imageUrl :url })
...
<form class="grid grid-rows ">
  <div class="grid grid-cols-12" style="padding: 128px;">
  ...
  <div class="col-span-5  ">
    <div class="grid grid-row imageLayout">
      ...
      <div class="grid grid-cols-2  gap-4"> 
        <div v-for="item in coffeShop.image" id="" class="imageHolder">
          <img v-if="item.imageUrl" :src="item.imageUrl" />
        </div>
        <label class="custom-file-upload imageHolder">
          <input type="file" accept="image/*" @change="onChange" style=""/>
...
<button type="submit" style="margin-left: 410px;display: flex;width: 717px;height: 77px;padding: 16px 24px;justify-content: center;align-items: center;gap: 16px;flex-shrink: 0;border-radius: 8px;background: #36ABFF;color: #FFF;text-align: center;font-size: 20px;font-family: Inter;font-style: normal;font-weight: 700;line-height: normal;">
  + 喫茶店を追加
</button>
</form>
